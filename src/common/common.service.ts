...
import { ENV_HOST_KEY, ENV_PROTOCAL_KEY } from './const/env-keys.const';
...
export class CommonService {
  ...
  private async cursorPaginate<T extends BaseModel>(
    ...
    const protocol = this.configService.get<string>(ENV_PROTOCAL_KEY);
  ...
  private composeFindOptions<T extends BaseModel>(
     ...
     * 나중에 where__likeCount__more_than 이나 where__title__ilike 등 추가 필터를 넣고싶어졌을 때
     ...
     *         ['where', 'id', 'more_than']
     *    3-2) 2개의 값으로 나뉜다면 정확한 값을 필터하는 것이기 때문에 oprator 없이 적용한다.
     *         where__id
     *         ['where', 'id']
     * 4) order의 경우 3-2와 가팅 적용한다.
     */
